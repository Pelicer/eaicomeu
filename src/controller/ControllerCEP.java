...
public final class ControllerCEP {

	private enum Xml {
		CIDADE {
			@Override
			public void setCep(String text, ControllerCEP webServiceCep) {
		...
		public abstract void setCep(String text, ControllerCEP webServiceCep);
	}
	...
	public static ControllerCEP searchCep(String cep) {
		cep = cep.replaceAll("\\D*", "");
		if (cep.length() > 8)
			cep = cep.substring(0, 8);
		ControllerCEP loadCep = new ControllerCEP(cep);
		try {
	...
	private ControllerCEP(String cep) {
		this.cep = cep;
	}
